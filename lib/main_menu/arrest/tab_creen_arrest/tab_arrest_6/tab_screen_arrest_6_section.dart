...
class _TabScreenArrest6SectionState extends State<TabScreenArrest6Section>  {
  ...
  Widget _buildContent(BuildContext context) {
    ...
    TextStyle textInputStyleSub = TextStyle(
        fontSize: 14.0, color: labelColor, fontFamily: FontStyles().FontFamily);
    TextStyle textPreviewStyle = TextStyle(fontSize: 14.0,
        color: labelPreview,
        fontFamily: FontStyles().FontFamily);
            ...
            child: Column(
              ...
              children: <Widget>[
                  ...
                  children: <Widget>[
                    Expanded(
                      child: Container(
                        ...
                        child: Text(
                          ItemsSuspect[index].TITLE_SHORT_NAME_TH.toString() +
                              ' ' +
                              ItemsSuspect[index].FIRST_NAME + " " +
                              ItemsSuspect[index].LAST_NAME,
                ...
                ItemsSuspect[index].MISTREAT_NO == 0
                  ...
                  child: Text(
                    "ยังไม่เคยกระทำผิด " +
                        ItemsSuspect[index].MISTREAT_NO.toString() + " ครั้ง",
      ...
      child: MaterialButton(
        onPressed: () {
          ...
          widget.ItemsProduct.length>0
              ...
              :Navigator.pop(context, widget.ItemsSection);

        },
        child: Center(
          child: widget.ItemsProduct.length>0
              ?(Text(_countItem==0?'ถัดไป':'ถัดไป (${_countItem})', style: textStyleButton,))
              :Text('ตกลง (${_countItem})', style: textStyleButton,),
        ),
      ),
    );
  }
  ...
  _navigateEvidence(BuildContext context) async {
    ...
    if (widget.ItemsProduct.length == 0) {
      ...
      result = await Navigator.push(
        context,
        MaterialPageRoute(builder: (context) =>
            TabScreenArrest6Evidence(IsUpdate: widget.IsUpdate,
              Title: widget.Title,
              ItemsData: widget.ItemsSection,
  ...
  Widget build(BuildContext context) {
    ...
    return new WillPopScope(
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: ItemsSuspect.length==0&&widget.ItemsProduct.length==0
            ? null
            :_buildBottom(),
      ),
    );
